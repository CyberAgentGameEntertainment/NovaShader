...
One case skips the resume on purpose: if the marker had **already** hit by the time the wait confirmed its arming (a single-shot marker, or any marker whose mode does not wait for a new hit — including one just armed by the same `enable-pause-point --await` call), the wait settles on that recorded hit instantly, so resuming would restart the game and then report an old hit. The wait returns the recorded hit and reports `ResumePlayResult.Skipped` (with `Resumed: false` and no `Error`); a `--trigger` given alongside is not dispatched and says so in `TriggerResult.Error`. Play Mode is left as it is (still paused when that hit paused it) — clear or re-enable the marker and wait again to capture a new one. An already-hit `continuous`/`trace` marker that was armed earlier is unaffected: its wait needs a later hit, so it still resumes.
