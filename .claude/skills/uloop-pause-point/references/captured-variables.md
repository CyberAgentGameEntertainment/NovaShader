...
- By default (`--snapshot-timing pre-line`) the snapshot is taken **before** the resolved line executes, exactly like an IDE breakpoint on that line. The enable response's `SnapshotTiming` field states which timing is active.
- `--snapshot-timing post-line` captures **after** the resolved line's own statement finished, just before control leaves it. Use it to read the value an assignment or call on that line produced without arming the next line — which may be a join point that an early `return` or the `else` branch also reaches, so a marker there fires for executions that never ran the requested line. Post-line semantics per statement shape:
  - `if (cond)` / `while (cond)` when the body starts on another line: the capture runs after the condition was evaluated, whichever way the branch goes; it never runs inside the body. For a single-line `if (cond) statement;`, the capture runs at the join point after the body, so mutations from `statement` are visible when the condition is true. When that body exits instead of reaching the join point, as in `if (cond) return;`, the capture stays before the conditional branch so both outcomes can reach it. The same holds when the branch stays inside the line, as in `if (cond) a = 1; else b = 2;` or `if (cond) a = 1; b = 2;` on one line: the capture runs right after the condition, before any of those statements. A `for` header resolves to its initializer, so the capture runs after `i = 0` and before the first condition check; loop iterations are not captured.
  - `return expr;`: the capture runs after `expr` was evaluated, before the method returns (the value itself is not a named local unless you assigned one).
  - A line that always throws (`throw new ...;`) is rejected at enable time with a resolve failure; pick another line or `pre-line`.
  - `await` lines: the capture runs once the awaited call has been issued, before the completion check — the method may or may not suspend afterwards, and the capture never sees the resumed state; put a `pre-line` marker on the next statement for that.
  - Multiple statements on one line (`a = 1; b = 2;`) count as one line: the capture runs after all of them.
- The pause itself only takes effect at the next frame boundary: the frame that hit the pause point still runs to completion first, so any event that fires later in that same frame (a chained collision, a cascading destroy) has already happened by the time Unity actually stops. Trust `CapturedVariables` (the snapshot at the chosen timing) as evidence for what was true up to the patched line; do not assume the paused state still matches it for events later in the same frame.
